...
+ "CXX = " + whichCPP + "\n"
+ format("CFLAGS = -std=c++%i -Wall%s -g -std=c++17 -DPROJECT_NAME=\"\\\"%s\\\"\" -DPROJECT_VERSION=\"\\\"%s\\\"\" -DPROJECT_AUTHOR=\"\\\"%s\\\"\" %s", (int)cppVersion, (cppStrict ? " -Werror -Wpedantic" : ""), commands::escape_quotes(commands::escape_quotes(projectName)).c_str(), commands::escape_quotes(commands::escape_quotes(projectVersion)).c_str(), commands::escape_quotes(commands::escape_quotes(projectAuthor)).c_str(), std::string(TYPE == BuildType::TEST ? std::string(" -include ") + BUILD_DIR + "/__Testing__.hpp" : "-D'TEST(...)='").c_str()) + featureFlags + " " + R"""(
